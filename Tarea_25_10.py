...
tamaño_matriz = int(input("Ingrese el tamaño de la matriz: "))
m = [list(map(int, input().split())) for _ in range(tamaño_matriz)]
s = sum(m[0])

magica = True
for i in range(tamaño_matriz):
    if sum(m[i]) != s or sum(m[j][i] for j in range(tamaño_matriz)) != s:
        magica = False

if sum(m[i][i] for i in range(tamaño_matriz)) != s or sum(m[i][tamaño_matriz-1-i] for i in range(tamaño_matriz)) != s:
    magica = False

print("Es una matriz mágica" if magica else "No es una matriz mágica")
